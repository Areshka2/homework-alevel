...
function cardTemplate(pizza) {
  ...
  pizzaItem.className = "pizza-item col mb-4";
  ...
  if (pizza.isFavorite) btnFavorite.classList.add('btn-favorite--active');
  ...
  btnAddToCart.textContent = "Купить";

  let pathToImg = pizza.img.includes('base64') ? pizza.img : `img/${pizza.img}`
...
// function add pizza to cart
function addToCart(pizza) {
  let findPizzaById = cart.find(item => item.id === pizza.id)
...
// get total price and count for cart
const totalOptions = () => {
...
// if the product is in the cart, go to the order page
document.querySelector('.btn-cart').onclick = function (e) {
  e.preventDefault();

  if (totalOptions().totalCount) {
    window.open('http://127.0.0.1:5500/js/17/order.html', '_self');
  }
}